...
const ProcessStatus = ({
    ...
    return (
        submissionStatus === 6 && (JSON.parse(authProfile)?.role === "teknis_teknologi" || JSON.parse(authProfile)?.role === "katim_teknologi" ?
            <div className="flex flex-col gap-3">
                {JSON.parse(authProfile)?.role === "teknis_teknologi" && (
                ...
                {JSON.parse(authProfile)?.role === "katim_teknologi" && (
                    Object.entries(processData).length !== 0 ?
                        <div className="flex flex-col gap-3">
                            <div className="flex flex-1 flex-col gap-3 bg-lightColor dark:bg-cardDark p-3 rounded-lg">
                                <span className='text-lg font-bold'>Tahapan Proses</span>
            ...
            <div className='flex flex-col lg:flex-row gap-3'>
                ...
                <DynamicDetails location={"teknologi sistem informasi"} detailData={detailData} loading={loading} />
            </div>
        )
    );
};
