...
function LayananTeknologiSIPages() {
  ...
  const [listTeknologiSistemInformasi, setListTeknologiSistemInformasi] = useState([]);
  const [listTeknologiSstemInformasiLoading, setListTeknologiSistemInformasiLoading] = useState(true);
  ...
  useEffect(() => {
    if (authToken) {
      fetchDataTeknologiSistemInformasi(authApiKey, authToken, JSON.parse(authProfile)?.role)
  ...
  const fetchDataTeknologiSistemInformasi = async (api_key, token, role) => {
    setListTeknologiSistemInformasiLoading(true);
    ...
    try {
      ...
      if (response?.statusCode === 200) {
        if (JSON.parse(authProfile)?.role === "perangkat_daerah") {
          ...
          setListTeknologiSistemInformasi(filteredSubmissions);
        } else {
          setListTeknologiSistemInformasi(response.result.data);
        }
        ...
        setListTeknologiSistemInformasi([]);
      }
    } catch (error) {
  ...
  const handleImageUploadAndFetch = async (obj) => {
    if (obj.image_screenshoot) {
      const result = await fetchUploadImages(authApiKey, authToken, obj.image_screenshoot, 'Layanan teknologi dan sistem informasi', dispatch);
      if (result !== null) {
        const fixObject = {
          ...obj,
  ...
  return (
    <div className="flex flex-col gap-3 flex-1 p-4" >
      <TitleHeader title={JSON.parse(authProfile)?.role === "perangkat_daerah" ? "Layanan Pengajuan" : "Layanan Teknologi dan Sistem Informasi"}

        link1={"dashboard"}
        link2={'Bidang Teknologi dan Sistem Informasi'} />
        ...
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex md:flex-row flex-col gap-3">
            {(JSON.parse(authProfile)?.role === "perangkat_daerah") &&
              ...
              >
                <span className="sm:text-xl text-sm text-darkColor font-semibold">Selamat datang di Layanan Teknologi dan Sistem Informasi</span>
          ...
          <div className="flex flex-col gap-2 bg-lightColor dark:bg-cardDark p-3 rounded-lg">
            ...
            <div className="flex flex-col relative">
              <TableCostum
                ...
                data={listTeknologiSistemInformasi}
              />
            </div>
          </div>
        </div>
...
export default LayananTeknologiSIPages;
