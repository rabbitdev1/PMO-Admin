...
export  const convertToNameValueObject = (obj) => {
  ...
  obj.fields.forEach((field) => {
    if (field.type === 'selection') {
      const selectedOption = field.options.find((option) => option.value === field.value.value);
      ...
      if (field?.field) {
        const filteredArray = filterByTypeSelect(field?.field, field?.value?.value);
        const transformedArray = filteredArray.map(item => ({ name: item.name, value: item.value, type: item.type }));
      ...
      const valuesArray = field.value.map(item => item.value);
      result[field.name] = valuesArray
    }
