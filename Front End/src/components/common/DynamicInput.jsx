  ...
  const renderDocument = (src, type) => {
    switch (type) {
      case 'application/pdf':
        return <embed src={src} type="application/pdf" className="w-full h-96" />;
      case 'application/msword':
      case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
        return <iframe src={`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(src)}`} className="w-full h-96" frameBorder="0"></iframe>;
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-2">
              <div
                className={`flex flex-row gap-2 bg-lightColor dark:bg-darkColor text-lightColor dark:text-darkColor items-center  p-2.5 ${className} rounded-lg border-1 border-[#dddddd] dark:border-[#ffffff20]`}
              >
                <input type="file" onChange={handlePdfChange} accept=".pdf, .doc, .docx, .xls, .xlsx, .ppt, .pptx"
                ...
                <div key={index} className=" flex flex-row gap-3 items-center">
                  ...
                  {item.type === "selection" ?
                    <Select
                      ...
                      onChange={(selected) => {
                        const updatedValue = [...value];
                        updatedValue[index] = { ...updatedValue[index], value: selected };
                        onChange(updatedValue);
